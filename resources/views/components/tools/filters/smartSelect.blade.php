...
<div id="smartSelect-{{ $filterKey }}" x-data="{
    ...
    swapLabels() {
        document.getElementById('{{ $tableName }}-filter-{{ $filterKey }}-label2').classList.remove('hidden');
                        ...
                        <div class="pt-1 z-50">
                            ...
                            <div class="w-full">
                                ...
                                <a href="#" x-on:click="popOpen = false"
                                    class="inline-block float-right text-right px-1 py-1 pr-2 text-base" role="menuitem"
                                    tabindex="-1" id="menu-item-0">
                                    <svg class="inline" width="2em" height="2em" viewBox="0 0 24 24"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <rect width="24" height="24" />
                                        <path fill-rule="evenodd" clip-rule="evenodd" fill="#F0F0F0"
                                            stroke="currentColor"
                                ...
                                <template x-for="(item,index) in newSelectedItems">
                                    <div class="w-full pl-2 py-1 text-sm hover:bg-blue-500 dark:hover:bg-gray-400"
                                        ...
                                        <a href="#" class="inline-block w-full"
                                            x-on:click="removeSelectedItem(item.id)">
                                            <svg @class([
                            ...
                            <template x-for="(filteredItem, index) in currentFilteredList" :key="filteredItem.id">
                                <li class="px-2 py-1 hover:bg-blue-500 dark:hover:bg-gray-400"
                                        ...
                                        <a class="cursor-pointer inline-block w-full"
                                            ...
                                            @if ($iconStyling['delete']['svgEnabled'])
                                                <svg @class([
                                                    ...
                                                    width="{{ $iconStyling['delete']['svgSize'] }}"
                                                    height="{{ $iconStyling['delete']['svgSize'] }}" version="1.1"
                                                    fill="currentColor" viewBox="0 0 297 297" xml:space="preserve"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="m150.33 203.76c0-32.35 26.317-58.667 58.667-58.667 6.527 0 12.8 1.087 18.669 3.063l4.882-58.587h-185.39l14.518 174.21c1.552 18.627 17.41 33.219 36.103 33.219h84.147c18.692 0 34.551-14.592 36.103-33.219l0.173-2.081c-3.001 0.475-6.075 0.729-9.207 0.729-32.349 0-58.667-26.317-58.667-58.667z" />
                                                    <path
                                    ...
                                    <template x-if="selectedItems.indexOf(filteredItem.id.toString()) < 0">
                                        <a class="cursor-pointer inline-block w-full"
                                            x-on:click="addSelectedItem(filteredItem.id.toString())">
        ...
        <div>
            ...
            <div class="rounded-md position-relative">
                <div x-data="{ popOpen: false }" x-cloak>
                    <div class="w-100 h-100 md:top-0 text-right   ">
                        <button @click="popOpen = ! popOpen" type="button" style="margin-top: -1.6em;"
                            class="position-relative  bg-white dark:bg-gray-700 mt-n5 w-5 h-5  rounded-full items-left text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-200 z-0"
                            id="menu-button" aria-expanded="true" aria-haspopup="true">
                            <span class="sr-only">Open options</span>
                            <!-- Heroicon name: solid/dots-vertical -->
                    ...
                    <div x-show="popOpen" @click.outside="popOpen = false" x-transition
                        class="float-right position-absolute bg-white dark:bg-gray-700 ml-5"
                        style="width: 150px; left: 80px;" role="menu" aria-orientation="vertical"
                        aria-labelledby="menu-button" tabindex="-1">
                        <div class="pt-1 z-50 text-right">
                            <!-- Active: "bg-gray-100 text-gray-900", Not Active: "text-gray-700" -->
                            <a href="#" x-on:click="popOpen = false"
                                class="text-gray-700 inline-block w-100 text-right px-4 py-2 text-lg" role="menuitem"
                                tabindex="-1" id="menu-item-0">
                                <svg class="inline" width="2em" height="2em" viewBox="0 0 24 24"
                            ...
                            <div class="flex flex-col gap-y-2 text-left">
                                <template x-for="(key,index) in selectedItems">
                                    <div class="w-100 pl-2 text-sm">
                                        <a href="#" class="smartSelectIcon"
                                            x-on:click="removeSelectedItem(key)">
                                            <svg @class([
                                                'inline-block' => $iconStyling['delete']['defaults'],
                                                $iconStyling['delete']['classes'],
                                            ])
                                        ...
                                        <span x-text="filteredList[key].name"></span>
            ...
            <div class=" w-100 flex flex-col items-center justify-center ">
                <input x-on:keyup="updateCurrentFilteredList" type="text" x-ref="{{ $xRefKey }}"
                    ...
                    class=" w-100 border-gray-300 rounded-md dark:bg-gray-800 dark:text-white dark:border-gray-600">
                <div class="w-100 overflow-visible z-50 ">
                    <ul class="list-unstyled bg-white dark:bg-gray-800 w-100">
                        <template x-for="filteredItem in currentFilteredList" :key="filteredItem.id">
                            <li @class([
                                'cursor-pointer w-100 text-gray-700 text-black dark:text-white bg-white dark:bg-gray-700 hover:bg-sky-700 dark:bg-darker hover:dark:bg-sky-700' =>
                                    $listStyling['defaults'],
                                $listStyling['classes'],
                            ])>
                                <template x-if="selectedItems.indexOf(filteredItem.id.toString()) > -1">
                                    <a class="cursor-pointer smartSelectIcon"
                                        x-on:click="removeSelectedItem(filteredItem.id.toString())">
                                        @if ($iconStyling['delete']['svgEnabled'])
                                            <svg @class([
                                                'inline-block' => $iconStyling['delete']['defaults'],
    ...
    @elseif ($theme === 'bootstrap-5')
        <div>
            <!-- Start Graph API Pin -->
            <div class="rounded-md position-relative">
                <div x-data="{ popOpen: false }" x-cloak class="ms-2">
                    <div class="w-100 h-100 md:top-0 text-right   ">
                        <button @click="popOpen = ! popOpen" type="button" style="margin-top: -1.6em;"
                            class="position-absolute end-0 bg-white  mt-n5 w-5 h-5  rounded-full items-left text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-200 z-0"
                            id="menu-button" aria-expanded="true" aria-haspopup="true">
                    ...
                    <div x-show="popOpen" @click.outside="popOpen = false" x-transition
                        class=" position-absolute start-100  ms-2" style="width: 150px;" role="menu"
                        aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                        <div class="z-50 bg-white mx-1 text-right">
                            <!-- Active: "bg-gray-100 text-gray-900", Not Active: "text-gray-700" -->
                            <a href="#" x-on:click="popOpen = false"
            ...
            <div class=" w-100 flex flex-col items-center justify-center ">
                <input x-on:keyup="updateCurrentFilteredList" type="text" x-ref="{{ $xRefKey }}"
                    ...
                    class=" w-100 border-gray-300 rounded-md dark:bg-gray-800 dark:text-white dark:border-gray-600">
                <div class="w-100 overflow-visible z-50 ">
                    <ul class="list-unstyled bg-white dark:bg-darker w-100">
                        <template x-for="filteredItem in currentFilteredList" :key="filteredItem.id">
                            <li @class([
                                'cursor-pointer w-100 text-gray-700 text-black dark:text-white bg-white hover:bg-sky-700 dark:bg-darker hover:dark:bg-sky-700' =>
                                    $listStyling['defaults'],
                                $listStyling['classes'],
                            ])>
                                <template x-if="selectedItems.indexOf(filteredItem.id.toString()) > -1">
