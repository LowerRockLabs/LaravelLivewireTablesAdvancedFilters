@php
    ...
    if (isset($this->{$component->getTableName()}['filters'])) {
        if (isset($this->{$component->getTableName()}['filters'][$filter->getKey()])) {
            $filterMin = isset($this->{$component->getTableName()}['filters'][$filter->getKey()]['min']) ? $this->{$component->getTableName()}['filters'][$filter->getKey()]['min'] : 0;
            $filterMax = isset($this->{$component->getTableName()}['filters'][$filter->getKey()]['max']) ? $this->{$component->getTableName()}['filters'][$filter->getKey()]['max'] : 100;
        }
    }
...
<div class="mt-4 h-22 pt-8 pb-4 grid gap-10">
    <div class="range-slider flat" data-ticks-position='bottom'
        style='--min:0; --max:100; --value-a:{{ $filterMax }}; --value-b:{{ $filterMin }}; --suffix:"%"; --text-value-a:"{{ $filterMax }}"; --text-value-b:"{{ $filterMin }}";'>
        ...
        <output></output>
        <input type="range" min="0" max="100"
            wire:model="{{ $component->getTableName() }}.filters.{{ $filter->getKey() }}.min"
